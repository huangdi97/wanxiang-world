...
| 0113 | G35F: narrative/household/historical-China domain = generic Domain Pack (narrative.actions/components/resolver/query); FORBIDDEN RedChamberCore (no proper nouns, tested); missing actions live in the pack, never Core reference actions; all writes flow through Commit Authority; access/visit/letter/relay/duty reuse institution + material abstractions | accepted |
| 0114 | G35G: character knowledge-boundary distillation = CharacterDistiller (pure) + CharacterFact/RelationClaim (public/private scope, kind, temporal) + KnowledgeBoundary (private facts of others hidden unless granted; future never runtime); Completion/Interpretive separated; anonymized keys prove identity-agnostic mechanism; reuses G35E scenario/temporal + G35B locators | accepted |
| 0115 | G35H: completion ledger review = CompletionRecord (support refs/confidence/review status) + CompletionReviewLedger (E0-E5 stage semantics; E4/E5 terminal, never auto-upgrade to E0; can_enter_canon default FALSE, only human/rule review + evidence lifts it) + CompletionStudio + batch CLI; conflicts preserved (submit never overwrites, id reuse refused); HTTP router deferred to world-pack surface | accepted |
| 0116 | G35I/M32: WorldPackAssembler compiles v5.2 world manifest (constitution/genesis/evolution/lineage refs) + WorldDefinition + ScenarioPoint with HMAC signature and deps; generic literary constitution; instantiation dry-run only (no authority writes); core packages must stay free of Red Chamber hardcode; M32 gate = MECHANISM PASS (real corpus EXTERNAL_BLOCKED until legal edition) | accepted |
| 0117 | G36A: RC-001 instantiation = genesis delta committed through the single Commit Authority (1 event) + immutable initial snapshot + lineage root node; synthetic anonymized content (real canon EXTERNAL_BLOCKED); no second instantiation/registry path | accepted |
| 0118 | G36B: RC-001 spatial run reuses G02A spatial graph; movement time added as MovementProfile/travel_time (pure); visibility/acoustic/privacy/access verified on synthetic map; locked portals block, private places require grant | accepted |
| 0119 | G36C: RC-001 NPC policy = NPCProfile + daily_schedule (deterministic) composing institution duty / narrative visit / body rest; population resolution under capacity; actions flow through existing resolvers; no direct writes | accepted |
| 0120 | G36D: read-and-remember = existing material read transition + epistemic observation memory (reading forms memory; custody never equals knowledge); reuses G04E material resolvers + G35F letter domain; no new store | accepted |
...
| 0138 | M42: production release mechanism certified (982+1 PASS); SDK/package/API frozen; final certification = V5_2_PRODUCTION_PASS (RED_CHAMBER_REAL EXTERNAL_BLOCKED); local tag m42-v5.2-production; no push/deploy; v5.3 not started | accepted |
| 0139 | M57: compiler consumes revisioned WorldDraft with exact source/domain pins; PackageAssembler reuses PackageManifest; preview is isolated and uses the existing WorldHost/Runtime/Commit Authority; completion gaps never become E0 Canon | accepted |
| 0140 | M58: Studio/API/CLI converge on one no-API AuthoringService; source/job checkpoints, review, package and preview remain proposal/Forge paths; OCR without a provider is explicitly OCR_REQUIRED | accepted |
| 0141 | M59: resolved source bytes must match immutable hashes and pass ingestion security before the shared adapter pipeline; changed job fingerprints require a new version/id; E0 sources remain non-compilable | accepted |
| 0142 | M60: book-scale semantic views are deterministic Candidate analysis with reversible identity/coreference, explicit time uncertainty, topology without coordinates, object/institution observations, and benchmark-only quality proxies | accepted |
| 0143 | M61: source families pin editions/roles/versions; fusion exposes alignments, provenance, dissent, conflict impact, incremental affected keys, and rights-excluded candidates without selecting Canon | accepted |
| 0144 | M62: OCR/vision/ASR and external connectors are optional proposal ports; missing capabilities are explicit, subtitles/bundles are deterministic metadata, and no network/provider path writes Canon | accepted |
| 0145 | M63: domain inference emits explainable fingerprints, composition locks, gap/capability candidates, consent-gated reuse, and application-level validation only; no world-specific logic enters Kernel | accepted |
| 0146 | M64: missingness is a dependency graph, completion emits only E1-E5 candidates, consistency checks cover temporal/identity/topology/ownership/knowledge/organization/scenario/package/rights, and uncertainty remains calibrated metadata | accepted |
| 0147 | M65: scenario/genesis authoring emits three deterministic modes with immutable snapshot/activation/policy/profile metadata; seed is source-version-derived; no plan mutates runtime Canonical World State | accepted |
| 0148 | M66: worldness validation uses a bounded deterministic reference simulation; failures localize to Forge layers; repair/recompile remains candidate-only; branch and determinism proofs do not write Canonical World State | accepted |
| 0149 | M67: authoring uses one topological DAG with explicit stage policy, provider selection constraints, bounded stop/budget decisions, and existing JobService checkpoints for resume; provider routing never owns Commit Authority | accepted |
| 0150 | M68: review impact is deterministic over downstream/uncertainty/conflict/rights signals; auto-approval is policy-gated; batch `keep_unknown` maps to defer; inbox/API/audit reuse the existing ReviewLedger and never promote Canon | accepted |
| 0151 | M69: one-click Book/GEDCOM/JSON/CSV/Mixed reuse AuthoringService and PackageManifest; publish is an explicit Forge checkpoint after PackageValidator, preview/runtime entry uses existing WorldHost/Commit Authority, and missing/rights/OCR states remain explicit | accepted |
| 0152 | M70/G73A: clean-room certification is the deterministic synthetic/bootstrap boundary; migration, replay, backup/restore and external sample-pack evidence may PASS without importing copyrighted books or family-private records | accepted |
| 0153 | M70/G73B: repository safety permits only the red-chamber README/manifest template; source bytes, private records, tokens, model caches and rights-sensitive artifacts remain untracked and external | accepted |
| 0154 | M70/G73C: capacity evidence remains the deterministic SQLite/synthetic profile; report measured throughput and replay numbers without promoting them to real-book, PostgreSQL-PITR, or multi-node production claims | accepted |
| 0155 | M70/G73D: public examples must call the existing CLI/API/ProviderRouter/DomainRegistry/RightsGate surfaces; examples use synthetic data and keep OCR_REQUIRED, proposal-only, E0, and Commit Authority boundaries explicit | accepted |
| 0156 | M70/G73E: generated OpenAPI/SDK contracts are refreshed from the server source of truth; restricted Windows temp paths are replaced only in test fixtures by workspace-local scratch, while the SQLite/PostgreSQL production boundary remains unchanged | accepted |
| 0157 | M70/G73F: a real remote CI failure must be reproduced, fixed, pushed, and re-run green before the gate is PASS; cross-platform filesystem traversal is normalized in the architecture scan rather than allowlisting a platform-specific order | accepted |
| 0158 | M70/G73G: create v5.4.0-rc1 only after tag absence and required Actions green are verified; use an immutable annotated tag at the qualified HEAD, never move v5.3.0-rc1, and keep RC limitations explicit | accepted |
| 0159 | M70/G73H: final acceptance covers the complete M51-M70 chain and records local/remote evidence plus rights/OCR/E0/Commit Authority boundaries; after PASS the execution must stop before model training, M71, or v5.5 | accepted |
