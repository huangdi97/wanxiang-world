...
| 2026-08-15 | G35A | Real Red Chamber full-text source unavailable in environment | No legal/traceable edition; read-only sandbox + no network/rights verification | EXTERNAL_BLOCKED; registration mechanism verified with synthetic fixture |
| 2026-08-29 | G103D | First two isolated-clone attempts could not create the Git worktree under the host temp/workspace path; the first runner also recalculated bootstrap failure as PASS | Windows sandbox/Git subprocess permission boundary plus an evidence aggregation bug | Preserved `clean_clone_attempt_01/02` artifacts and reports; runner now fails closed and the real standard-temp clone passed |
| 2026-08-29 | G103E | `git ls-remote` cannot invoke the host HTTPS remote helper; GitHub API confirms no candidate release branch or candidate Actions run | Host Git installation lacks `remote-https`; no remote candidate delivery was performed | `EXTERNAL_BLOCKED`/`LOCKED`; no workaround, force push, tag, or Release |
