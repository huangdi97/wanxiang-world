...
## v5.5 Stable Certification — G98A / M95 (2026-08-28)

G98A passed the local Gate 61 baseline: RC1, Final Closure, and v5.4 tag
lineage are intact, the dedicated branch is
`release/v5.5-stable-certification`, and architecture/kernel/playable/Studio/
release/focused regression probes passed. Evidence is generated by
`scripts/stable_baseline.py` at
`artifacts/v55_stable/baseline.json`; the new Stable ledger is
`reports/V55_STABLE_ACCEPTANCE_MATRIX.md`. The live Git fetch is retained as
`EXTERNAL_BLOCKED` due to the host HTTPS remote-helper failure, and the named
v5.5 master filename was not found in the checkout. Human-player Gates 62–66
are `USER_INPUT_REQUIRED`: G98B/G98C automated route evidence passed, but no
genuine human ratings/notes are present. The build-generated packet and M95
...
## v5.5 G88B checkpoint (2026-08-26)

G88B PASS: PlayableWorldProfile schema 1, v0 compatibility, visibility
...
## v5.5 G89C checkpoint (2026-08-26)

G89C PASS: the existing Epistemic substrate now has perception-source refs,
...
## v5.5 G93F checkpoint (2026-08-27)

G93F PASS: local/global and observer-specific reputation views now update only
from committed-event/observation evidence through bounded, reviewed proposals;
belief and rumor are rejected. Social roles are separate reviewed projections,
not institution roles or permissions. The real private source → WorldPackage →
Preview → PlayableService → SQLite Runtime chain supplied a committed event,
