"""G94C: candidate evaluation consumes real committed playable evidence."""
...
def _source() -> SourceRecord:
    content = (
        ...
        "relationship: Alice -> Bob\nrule: repeated practice is evidence, not truth\n"
    )
    return SourceRecord(
        source_id="g94c_candidate_source",
        kind="text",
        content_hash=payload_hash(content),
        content_ref="memory://g94c_candidate_source",
        stage="E3",
...
def test_habit_candidate_is_read_only_over_real_product_chain(
    persist_db_path: pathlib.Path,
) -> None:
    source = _source()
    package = OneClickAuthoring().run("job_g94c_candidate", (source,), profile="book").package
    ...
    profile = playable.register_package(package, owner_id="g94c_owner", visibility="private")
    character = playable.entry.create_character(
        "g94c_owner",
        "Alice",
        compatible_profile_ids=(profile.experience_package_ref,),
        character_id="character_g94c_alice",
    )
    entered = playable.enter(
        profile.profile_id,
        viewer_id="g94c_owner",
        mode="embodiment",
        session_id="session_g94c_alice",
    ...
    actor = EntityId(_actor_ids(runtime.current_state(instance, branch))["Alice"])
    for _ in range(3):
        playable.action(
            instance.value,
            viewer_id="g94c_owner",
            action_type="set_status",
            payload={"entity_id": actor.value, "status": "watching"},
        )
    before = runtime.current_state(instance, branch)
    events = runtime.events(instance, branch)
    observations = PatternObservationStore.rebuild(events, window_size=100)
    detection = next(
        item
    ...
    candidate_policy = HabitPromotionPolicy(
        minimum_occurrences=3,
        minimum_windows=1,
    ...
    assert evaluate_habit_candidate(
        candidate, now_ticks=detection.window_end, policy=candidate_policy
    ).eligible
