...
`PENDING_REMOTE_ACTIONS` at report creation. Local implementation, full
quality, and real-source product evidence pass. The final decision becomes
`ACCEPTED` only after the committed feature branch and all required GitHub
Actions jobs are green. Until then the historical
...
| M78 | G81A-G81J | Local PASS for G81A-G81F; G81G remote push/Actions, G81H version decision, G81I final evidence and G81J STOP remain conditional on the final remote gate. |
...
Do not create `v5.4.0-rc2` until the final report is changed to `ACCEPTED`
after remote required Actions pass. If that remote condition fails, keep
`v5.4.0-rc1` as the latest release and retain `NOT_ACCEPTED`/blocked status;
do not enter v5.5 or train a model.
